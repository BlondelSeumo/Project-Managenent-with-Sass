                    ...
                    <div class="mt-4">
                        ...
                        @if($currantWorkspace && $currantWorkspace->permission == 'Owner')
                            ...
                            <a href="#" class="btn btn-primary" data-ajax-popup="true" data-size="lg"
                               ...
                               data-url="{{route('projects.share.popup',[$currantWorkspace->slug,$project->id])}}"><i class="mdi mdi-email-outline"></i> {{ __('Share to Clients')}}</a>
                    ...
                    <div class="board" data-plugin="dragula" data-containers='{{json_encode($statusClass)}}'>
                        ...
                        @foreach($stages as $stage)
                            <div class="tasks animated">
                                <div class="mt-0 task-header text-uppercase">{{__(ucwords($stage->name))}} (<span class="count">{{count($stage->tasks)}}</span>)</div>
    ...
    <script>
            ...
            if(comment != ''){
                $.ajax({
                    ...
                    success:function (data) {
                        ...
                        toastr('{{__('Success')}}','{{ __("Comment Added Successfully!")}}','success');
                    },
                    error:function (data) {
        ...
        $(document).on("click",".delete-comment",function(){
            if(confirm('{{__('Are you sure ?')}}')) {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    success: function (data) {
                        toastr('{{__('Success')}}', '{{ __("Comment Deleted Successfully!")}}', 'success');
        ...
        $(document).on('submit','#form-subtask',function (e) {
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    toastr('{{__('Success')}}','{{ __("Sub Task Added Successfully!")}}','success');
        ...
        $(document).on("change","#subtasks input[type=checkbox]",function(){
            $.ajax({
                ...
                success: function(data)
                {
                    toastr('{{__('Success')}}','{{ __("Subtask Updated Successfully!")}}','success');
                    // console.log(data);
                },
                error: function(data)
                {
        ...
        $(document).on("click",".delete-subtask",function(){
            if(confirm('{{__('Are you sure ?')}}')) {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    success: function (data) {
                        toastr('{{__('Success')}}', '{{ __("Subtask Deleted Successfully!")}}', 'success');
        ...
        $(document).on('submit','#form-file',function (e) {
            ...
            $.ajax({
                ...
                error: function(data)
                {
                    data = data.responseJSON;
                    if(data.message) {
                        toastr('{{__('Error')}}', data.errors.file[0], 'error');
                        $('#file-error').text(data.errors.file[0]).show();
                    }
                    else{
                       toastr('{{__('Error')}}', '{{ __("Some Thing Is Wrong!")}}', 'error');
        ...
        $(document).on("click",".delete-comment-file",function(){
            if(confirm('{{__('Are you sure ?')}}')) {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    success: function (data) {
                        toastr('{{__('Success')}}', '{{ __("File Deleted Successfully!")}}', 'success');
