        ...
        @if($currantWorkspace || Auth::user()->type == 'admin')
            ...
            <div class="row">
                @foreach ($users as $user)
                    <div class="col-lg-6 col-md-6 animated">
                                ...
                                <div class="card-body p-0">
                                    <div class="card-header-action">
                                        <div class="dropdown card-widgets text-right">
                                            @if(Auth::user()->type == 'admin' && isset($user->getPlan))
                                                <div class="dropdown card-widgets">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        <a href="#" class="dropdown-item" data-ajax-popup="true" data-title="{{__('Change Plan')}}" data-url="{{route('users.change.plan',$user->id)}}"><i class="mdi mdi-settings mr-1"></i>{{__('Change Plan')}}</a>
                                                        <a href="#" onclick="confirm('{{__('Are you sure ?')}}')?document.getElementById('delete_user_{{$user->id}}').submit():'';" class="dropdown-item"><i class="mdi mdi-trash-can mr-1"></i>{{__('Delete')}}</a>
                                            ...
                                            @if($currantWorkspace && $currantWorkspace->permission == 'Owner' && Auth::user()->id != $user->id)
                                                <div class="dropdown card-widgets float-right">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        ...
                                                        <a href="#" onclick="(confirm('{{__('Are you sure ?')}}')?document.getElementById('delete-form-{{$user->id}}').submit(): '');" class="dropdown-item"><i class="mdi mdi-delete mr-1"></i>{{__('Remove User From Workspace')}}</a>
                                    ...
                                    <div class="profile-widget-name">{{$user->name}} @if(Auth::user()->type != 'admin')
                                            <div class="text-muted d-inline font-weight-normal">
                                                <div class="slash"></div> {{__($user->permission)}}</div>@endif</div>
